...
class ParticleApi
{
    ...
    //
    // Webhooks
    // https://docs.particle.io/guide/tools-and-features/webhooks/#introduction
    // At this time, the webhooks API is not documented, and is described
    // through the CLI only.
    // Webhooks listen for events from your devices and will make requests based
    // on those events.
    //

    /**
     * Gets a list of webhooks from the particle cloud.
     *
     * @return Psr\Http\Message\RequestInterface
     */
     ...
     * @return Psr\Http\Message\RequestInterface
     */
    public function createClaimCode($iccid = null, $imei = null)
    {
        $params = [];
     ...
     * @return Psr\Http\Message\RequestInterface
     */
    public function requestTransferDevice($deviceID)
    {
        return $this->claimDevice($deviceID);
    }
    ...
    // TODO: get streams of events (all events, your events, device events), and publish an event.

    //
    // Firmware
    // https://docs.particle.io/reference/api/#firmware
    //

    /**
     * Sets the particle core signal mode state.
     * Listed as "update a device" in the documentation.
     *
    ...
    public function flashSource($deviceID, $filename, $file)
    {
        return $this->uploadFirmware($deviceID, $filename, $file, false);
    }

    public function flashBinary($deviceID, $filename, $file)
    {
        return $this->uploadFirmware($deviceID, $filename, $file, true);
    }

    //
    // Organizations
    // https://docs.particle.io/reference/api/#organizations
    //
     ...
     * @return Psr\Http\Message\RequestInterface
     */
    public function removeTeamMember($orgSlug, $username)
    {
    ...
    //
    // Products
    // https://docs.particle.io/reference/api/#products
    //
    ...
    // TODO: Another four resource actions to add here.
    //
